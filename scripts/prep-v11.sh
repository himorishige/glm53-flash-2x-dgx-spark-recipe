#!/usr/bin/env bash
# Mac: Phase 1b prerequisites, safe to run concurrently with resume-after-dl.sh (WAN vs QSFP).
...
V11_DIGEST="sha256:4def0ef644cb2e9814136dcffd5e385e21bc594f48f3b292234051904abe85a6"   # ultima survey 2026-08-31 (tonyd2wild README)
TAG=$(echo "$V11" | tr '/:' '__')
...
n2 "docker image inspect --format '{{.RepoDigests}}' $V11 | grep -q $V11_DIGEST && echo '[prep] digest matches survey' || echo '[prep] WARNING digest differs from survey value'"
...
n1 'mkdir -p ~/models/GLM-5.3-Flash-DFlash2 && rsync -a --partial --inplace rsync://192.168.200.14:8873/dflash2/ ~/models/GLM-5.3-Flash-DFlash2/ && du -sb ~/models/GLM-5.3-Flash-DFlash2' && echo "DRAFTER_NODE1_OK"

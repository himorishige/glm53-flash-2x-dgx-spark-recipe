...
Config-as-code for serving `LibertAIDAI/GLM-5.3-Flash-NVFP4` (181 GiB) with vLLM tensor-parallel across two
NVIDIA DGX Spark (GB10, 121 GiB each) over the QSFP link. Every number here was measured on this exact
setup on 2026-09-01 (`docs/measurements.md`).

## Just use this

**Default = config S** — what `cluster.env.example` launches with no overrides.

| you want | run | measured |
| --- | --- | --- |
| **one user, long documents (default)** | `cluster.env` as shipped: 262K ctx / 2 slots / DFlash2 drafter k=7 / fp8 KV / marlin | 30.8 tok/s single, prefill ≈1.4K tok/s flat to 200K, needle recall at 200K ✅ |
...
| Japanese text that must be byte-exact | `OVERRIDES="MTP_NUM_TOKENS=0 LOGITS_PROCESSORS=utf8_guard_lp:Utf8GuardLogitsProcessor"` | 0 broken kanji, 14.6 tok/s |
...
the SM121 top-k patch mount (in `docker-compose.yml`), `--block-size 2304`, `--language-model-only`,
`--moe-backend marlin`, `--gpu-memory-utilization 0.85`, `--kv-cache-dtype fp8_e4m3`, `--enforce-eager`,
`--tool-call-parser glm47`, `--reasoning-parser deepseek_r1`. The DFlash2 drafter is CC-BY-NC-ND; for
commercial use take the MTP-4 row (26–27 tok/s).
...
`hf` CLI and `uv` for downloads. All commands run from a Mac that can ssh to both nodes (`node1` / `node2`
aliases in `~/.ssh/config`); the nodes need no keys for each other — inter-node copies go through a
read-only rsync daemon on the QSFP address.
...
(11.6 / 12.1 GB/s bus bandwidth here).
...
# verify the checkpoint (sha256 of all 123 LFS files against the Hub tree, ≈4 min per node)
...
## The effective `vllm serve` line (config S, rank 0)
...
cluster.env.example       every knob — defaults are config S
...
docs/measurements.md      every number we measured, with conditions — 14-row tuning sweep, bench vs a single Spark, long context × concurrency
docs/pitfalls.md          what did not work and why (image, KV dtype, MoE backends, env-file, benchmark hygiene, Japanese U+FFFD)
